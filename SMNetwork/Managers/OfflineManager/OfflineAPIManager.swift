...
typealias CompletionHandlerPendingRequest = (Bool) -> Void

class OfflineAPIManager {
    
    // Shared instance
    static let sharedInstance = OfflineAPIManager()
    ...
    public func registerForNetworkChange() {
    ...
    public func syncAllPendingAPIRequest() {
        ...
        if let pendingAPIList = PendingAPI.getPendingAPIs(predicate: nil, sort: [sort], context: context) {
            ...
            for pendingRequest in pendingAPIList {
                Logger.log(message: "Timestamp - \(pendingRequest.timeStamp)")
